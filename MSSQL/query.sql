...
BEGIN
	...
	SELECT @HOTEN = HO+ ' '+ TEN,@MALOP=MALOP FROM SINHVIEN WHERE MASV=@MASV AND PASSWORD=@PASSWORD AND (DANGHIHOC IS NULL OR DANGHIHOC =0)
	IF @HOTEN IS NOT NULL
		BEGIN
...
GO
-----------------------SITE 3 -----------------------
CREATE PROC [dbo].[SP_LayNienKhoaHP] 
AS
SELECT NIENKHOA FROM HOCPHI 
  GROUP BY NIENKHOA
GO
-----------------------SITE 3 -----------------------
CREATE PROC [dbo].[SP_LayHocKyHP]
@NienKhoa nchar(9)
AS
SELECT HOCKY FROM HOCPHI WHERE NIENKHOA=@NienKhoa GROUP BY HOCKY
GO
-----------------------SITE 3 -----------------------
CREATE PROC [dbo].[SP_LayTenKhoa]
@MaLop nchar(10)
AS
...
GRANT EXECUTE ON OBJECT::dbo.SP_LayNienKhoaHP TO PKT;
GRANT EXECUTE ON OBJECT::dbo.SP_LayHocKyHP TO PKT;
GRANT EXECUTE ON OBJECT::dbo.SP_LayTenKhoa TO PKT;
-----------------------SITE 1,2 -----------------------
...
GRANT EXECUTE ON OBJECT::dbo.SP_HUY_DKY_LTC TO SV;
